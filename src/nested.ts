...
export function getPublishedQuestions(questions: Question[]): Question[] {
    const publishedQuestionsList = questions.filter(
        (question: Question): boolean => question.published == true
    );
    return publishedQuestionsList;
}
...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    const populatedQuestions = questions.filter(
...
export function sumPoints(questions: Question[]): number {
    const totalOfPoints = questions.reduce(
...
export function toCSV(questions: Question[]): string {
    let CSVConversion = "id,name,options,points,published\n";
    CSVConversion += questions
        .map(
...
export function sameType(questions: Question[]): boolean {
    const sameTypeChecker = questions.every(
        (question) => question.type == questions[0].type
    );
    return sameTypeChecker;
}
...
export function changeQuestionTypeById(
    ...
    const changeQuestionType = questions.map(
        (question: Question): Question => ({ ...question })
    );
    for (let i = 0; i < changeQuestionType.length; i++) {
        if (changeQuestionType[i].id === targetId) {
            if (changeQuestionType[i].type === "multiple_choice_question") {
                changeQuestionType[i].options = [];
            }
            changeQuestionType[i].type = newQuestionType;
        }
    }
